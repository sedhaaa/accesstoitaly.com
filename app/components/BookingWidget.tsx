...
export default function BookingWidget() {
  ...
  // --- JEGYVÁLASZTÁS FIGYELÉSE (COMPARISON RÉSZRŐL) ---
  useEffect(() => {
    const handleTicketSelection = (e: any) => {
        const ticketId = e.detail;
        const variant = ticketVariants.find(v => v.id === ticketId);
        if (variant) {
            setSelectedVariant(variant);
            setStep(2); // Átugrás a naptárhoz
        }
    };
    window.addEventListener('select-ticket-variant', handleTicketSelection);
    return () => window.removeEventListener('select-ticket-variant', handleTicketSelection);
  }, [ticketVariants]);
  ...
  const initializePayment = async () => {
    ...
    const paymentInitData = { total, currency: 'eur' };
  ...
  return (
    ...
    <div className="bg-white w-full rounded-2xl md:rounded-3xl shadow-2xl border border-stone-100 relative z-20 text-[#1a1a1a] flex flex-col transition-all duration-300 overflow-hidden">
      ...
      <div className="p-4 md:p-8 min-h-[300px] md:min-h-[320px]">
          ...
          {/* STEP 4 - VALIDÁCIÓS ÜZENETEK VISSZATÉVE! */}
          {step === 4 && (
          ...
          {step === 5 && clientSecret && (
             <div className="step-animation pb-4">
                <Elements options={{ clientSecret, locale: locale as any, appearance: { theme: 'stripe', variables: { colorPrimary: '#B8860B', fontFamily: 'sans-serif' } } }} stripe={stripePromise}>
                    <StripePaymentForm total={total} customerDetails={formData} onSuccess={handlePaymentSuccess} onError={(msg: string) => alert(msg)} isProcessing={isProcessing} setIsProcessing={setIsProcessing} />
                </Elements>
             </div>
          )}
      </div>
      ...
      <div className="p-5 md:p-8 border-t border-stone-100 bg-white rounded-b-3xl">
        {serverError && (
            <div className="mb-4 bg-red-50 border border-red-200 rounded-xl p-3 flex items-start gap-3 error-anim">
                ...
                <p className="text-xs text-red-600 font-bold leading-relaxed">{t(`errors.${serverError.code}`, { time: serverError.param || '' })}</p>
            </div>
        )}
